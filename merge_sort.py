"""Program illustrates the merge sort algorithm with the effiency of O(n log(n))"""
def merge(items, sections, temp_storage):
    ...
    while i_1 < end_1 or i_2 < end_2:
        if i_1 < end_1 and i_2 < end_2:
            if items[i_1] < items[i_2]:
                temp_storage[i_t] = items[i_1]
                i_1 += 1
            else: #the_list[i_2] >= items[i_1]
                temp_storage[i_t] = items[i_2]
                i_2 += 1
            i_t += 1
    ...
    for i in range(i_t):
        items[start_1 + i] = temp_storage[i]
